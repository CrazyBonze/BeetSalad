...
# Needed for quote() — import here since we use it in mb_get above
import urllib.parse
...
def inspect_release_group(rgid: str) -> None:
    ...
    rg = mb_get(
        f"release-group/{rgid}",
        {"inc": "artists+releases+tags+ratings", "fmt": "json"}
    )
    ...
    tags = rg.get("tags", [])
    if tags:
        tag_list = ", ".join(f"{t['name']} ({t['count']})" for t in sorted(tags, key=lambda x: -x["count"])[:10])
        fmt("Tags:", tag_list, indent=1)

    rating = rg.get("rating", {})
    if rating:
        fmt("Rating:", f"{rating.get('value', '?')}/100 ({rating.get('votes-count', 0)} votes)", indent=1)
    ...
    if releases:
        ...
        rel_detail = mb_get(
            ...
            {"inc": "artists+labels+recordings+release-groups+media+artist-credits+isrcs", "fmt": "json"}
        )

        print("\n" + "-"*70)
        print(f"FIRST RELEASE — FULL DETAIL")
        print("-"*70)
        ...
        fmt("Language:", rel_detail.get("text-representation", {}).get("language", "unknown"))
        ...
        for disc in media:
            ...
            for t in tracks:
                ...
                print(f"      {t.get('number', '?'):>4}. {t.get('title', rec.get('title', '?')):<50} {length_str}  [{track_artist}]")
    ...
    if not primary_type:
        ...
        print("     This release group will be INVISIBLE to Lidarr unless your profile includes blank types.")
    ...
    if len(artist_names) > 1 or any("&" in n or " and " in n.lower() for n in artist_names):
        print(f"\n  ⚠  COLLABORATION DETECTED: Artist credit has multiple entities or uses '&'.")
        print(f"     Lidarr tracks albums under one artist. Check which MBID Lidarr uses for Frank Zappa:")
        for c in credits:
            if isinstance(c, dict) and "artist" in c:
                a = c["artist"]
                print(f"       Name: {a.get('name')}  MBID: {a.get('id')}")
        print(f"     In Lidarr, go to Frank Zappa's artist page and compare his MBID to the above.")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        print("  beet list -a -f '$album | $mb_releasegroupid' albumartist:'Frank Zappa'")
        sys.exit(1)
